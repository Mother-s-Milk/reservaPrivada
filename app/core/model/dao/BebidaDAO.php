...
final class BebidaDAO extends DAO implements InterfaceDAO
{
    ...
    //Devuelve solamente un entero con la cantidad total de bebidas registradas
    ...
    {
        $sql = "SELECT id, nombre, stock FROM {$this->table} WHERE stock <= 10
                    ORDER BY stock ASC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
    ...
    public function filter($data): array
    {
        ...
        if (!empty($data['nombre'])) {
            $conditions[] = "b.nombre LIKE :nombre";  // Alias agregado
        ...
        if (!empty($data['proveedor'])) {
            $conditions[] = "b.proveedorId = :proveedor";  // Alias agregado
            $params[':proveedor'] = $data['proveedor'];
        }

        // Filtro por categoriaId
        if (!empty($data['categoria'])) {
            $conditions[] = "b.categoriaId = :categoria";  // Alias agregado
